...
function SortableTask({
  ...
  const priorityConfig =
    task.priority === "High"
      ? {
          dot: "bg-red-400",
          badge: "bg-red-500/20 text-red-200 ring-red-400/60",
          edge: "border-l-red-400/70 shadow-[inset_3px_0_18px_rgba(248,113,113,0.22)]",
        }
      : task.priority === "Medium"
      ? {
          dot: "bg-amber-300",
          badge: "bg-amber-500/20 text-amber-200 ring-amber-400/60",
          edge: "border-l-amber-300/70 shadow-[inset_3px_0_18px_rgba(251,191,36,0.18)]",
        }
      : {
          dot: "bg-emerald-300",
          badge: "bg-emerald-500/15 text-emerald-200 ring-emerald-400/50",
  ...
  return (
    <motion.li
      ...
      className={`group relative flex items-start gap-2 rounded-xl border-l-2 bg-navy/80 px-3 py-2 ring-1 ring-white/10 transition-colors ${priorityConfig.edge} ${
    ...
    >
      <button
        ...
        className={`mt-1 grid h-4 w-4 place-items-center rounded border text-[10px] transition ${
          isDone
      ...
      <div className="flex-1 space-y-1 text-[11px]">
        <div className="flex items-center justify-between gap-2">
          <input
            ...
            className={`w-full border-none bg-transparent text-xs text-slate-100 outline-none transition ${
          ...
          <div className="relative flex items-center gap-1">
            <button
              ...
              className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] ring-1 transition ${priorityConfig.badge}`}
            ...
            <AnimatePresence>
              {priorityMenuOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 6 }}
                  className="absolute right-0 top-7 z-20 w-28 rounded-xl border border-white/15 bg-[#050711]/95 p-1 text-[11px] shadow-[0_16px_40px_rgba(15,23,42,0.95)]"
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                  }}
                >
                  {(["High", "Medium", "Low"] as const).map((p) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between rounded-lg px-2 py-1.5 text-slate-100 hover:bg-white/10"
                    >
        ...
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            className="min-w-[160px] flex-1 rounded-full bg-black/40 px-2 py-1 text-[11px] text-slate-100 outline-none ring-1 ring-white/15"
          />
          <span className="rounded-full bg-black/40 px-2 py-0.5 text-[10px] text-slate-300 ring-1 ring-white/15">
      ...
      <AnimatePresence>
        {confirmDeleteOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 6 }}
            className="absolute right-3 top-10 z-30 w-64 rounded-xl border border-white/15 bg-[#050711]/95 p-3 text-[11px] text-slate-100 shadow-[0_20px_55px_rgba(15,23,42,0.98)]"
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
            }}
          >
            <div className="font-semibold text-white">Delete this task?</div>
