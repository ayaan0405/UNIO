...
function statusConfig(status: EventStatus) {
  if (status === "upcoming") {
    return {
      label: "Upcoming",
      color: "bg-indigo-500 text-white border-indigo-400/50 shadow-lg shadow-indigo-500/25",
      icon: Clock3,
    };
  }
  if (status === "ongoing") {
    return {
      label: "Ongoing",
      color: "bg-amber-500 text-navy border-amber-400/50 shadow-lg shadow-amber-500/25",
      icon: BadgeCheck,
    };
  }
  return {
    label: "Completed",
    color: "bg-emerald-500 text-white border-emerald-400/50 shadow-lg shadow-emerald-500/25",
    icon: CheckCircle2,
  };
}
    ...
    <div className="space-y-5 p-6 text-slate-100">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Link
          ...
          className="inline-flex items-center justify-center rounded-full bg-indigo-500 px-4 py-2 text-xs font-semibold text-white shadow-lg shadow-indigo-500/40 ring-1 ring-indigo-400/60 hover:brightness-110 sm:text-sm"
        >
            ...
            const filterConfig =
              filter === "All"
                ? {
                    ...
                    inactive: "bg-slate-500/20 text-slate-200 hover:bg-slate-500/30",
                  }
                : filter === "Upcoming"
                ? {
                    active: "bg-indigo-500 text-white shadow-lg shadow-indigo-500/25",
                    inactive: "bg-indigo-500/20 text-indigo-200 hover:bg-indigo-500/30",
                  }
                : filter === "Ongoing"
                ? {
                    active: "bg-amber-500 text-navy shadow-lg shadow-amber-500/25",
                    inactive: "bg-amber-500/20 text-amber-200 hover:bg-amber-500/30",
                  }
                : {
                    active: "bg-emerald-500 text-white shadow-lg shadow-emerald-500/25",
                    inactive: "bg-emerald-500/20 text-emerald-200 hover:bg-emerald-500/30",
                  };
            ...
            return (
              <button
                ...
                className={`inline-flex items-center rounded-full px-4 py-2 font-medium transition-all hover:scale-105 ${
                  active ? filterConfig.active : filterConfig.inactive
                }`}
              >
                {filter}
              </button>
      ...
      {filteredEvents.length === 0 ? (
        <motion.div
          ...
          className="mt-6 flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-600/50 bg-black/40 px-6 py-12 text-center"
        >
          ...
          <p className="mt-2 max-w-md text-xs text-slate-400 sm:text-sm">
            Start by creating your first campus event.
          </p>
          <Link
            href="/dashboard/events/new"
            className="mt-5 inline-flex items-center justify-center rounded-full bg-indigo-500 px-4 py-2 text-xs font-semibold text-white shadow-lg shadow-indigo-500/40 ring-1 ring-indigo-400/60 hover:brightness-110"
          >
            <CalendarRange className="mr-2 h-4 w-4" />
            Create an event
          </Link>
        ...
        >
          {filteredEvents.map((event) => {
            ...
            return (
              // ✅ FIX: Use div + cursor-pointer instead of <Link> wrapper
              // This prevents nested <a> tags since the menu has a <Link> inside
              <motion.div
                key={event.id}
                variants={{
                ...
                className={`group relative cursor-pointer overflow-hidden rounded-2xl border border-white/10 shadow-[0_20px_55px_rgba(15,17,23,0.95)] backdrop-blur-sm transition ${
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <div className="flex items-center gap-1">
                        {/* Stop propagation so button clicks don't navigate */}
                        <button
                          type="button"
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      <div
                        className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold ${status.color}`}
                      >
                    ...
                    {/* Dropdown menu — uses router.push instead of <Link> to avoid nested <a> */}
                    <AnimatePresence>
                      {openMenu === event.id && (
                        <motion.div
                        ...
                        >
                          ...
                          {/* ✅ Use button + router.push instead of <Link> inside the card */}
                          <button
                            type="button"
                            onClick={() =>
                              router.push(`/dashboard/events/${event.id}`)
                      ...
                      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-1.5">
                          {event.assignees.map((initials) => (
                            <div
                              key={initials}
                              className="grid h-7 w-7 place-items-center rounded-full bg-white/90 text-[10px] font-semibold text-slate-900 shadow-[0_6px_16px_rgba(15,23,42,0.9)]"
                            >
                              {initials}
                            </div>
                          ))}
